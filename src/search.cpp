...
uint8_t pack(uint8_t bound, bool wasPv, uint8_t age) { return static_cast<uint8_t>(bound + (wasPv << 2) + (age << 3)); }

int     get_history_score(const Position& pos, SearchData* search_data, const Move& move, SearchStack* ss) { return 1; }

void    update_corrHistScore(const Position* pos, SearchData* search_data, int depth, int score) { return; }

void    update_histories(
       const Position* pos, SearchData* search_data, SearchStack* ss, int depth, const Move& best_move, MoveList* quietMoves,
       MoveList* noisyMoves) {
...
bool SEE(const Position pos, Move& move, int thresh_hold) {
    ...
    Bitboard bishops = Bitboard(
        side == WHITE ? pos.white_bishops.board() | pos.white_queens.board() : pos.black_bishops.board() | pos.black_queens.board());

    Bitboard rooks =
        Bitboard(side == WHITE ? pos.white_rooks.board() | pos.white_queens.board() : pos.black_rooks.board() | pos.black_queens.board());
...
template <bool pvNode> int search(int alpha, int beta, int depth, const bool cutNode, ThreadData* thread_data, SearchStack* ss) {
    ...
    if (!pvNode && tt_data.value != SCORE_NONE && tt_data.depth >= depth &&
        ((tt_data.bound == UPPER && tt_data.value <= alpha) || (tt_data.bound == LOWER && tt_data.value >= alpha) ||
         tt_data.bound == EXACT)) {
        return tt_data.eval;
    }
    ...
    if (inCheck) {
        ...
        if (tt_data.eval != SCORE_NONE && ((tt_data.bound == UPPER && tt_data.eval < eval) ||
    ...
    if (!pvNode && !excludedMove_val && !inCheck) {
        ...
        if (eval >= ss->staticEval && eval >= beta && (ss - 1)->move != NOMOVE && ss->ply >= thread_data->nmpPlies && no_pawns) {
            ss->move          = NOMOVE;
    ...
    while ((move = move_picker.next(skipQuiets)) != NOMOVE) {
        ...
        if (!isRootNode && no_pawns && bestScore > -MATE_FOUND) {
            const int lmrDepth =
                std::max(0, depth - reductions[isQuiet][std::min(depth, 63)][std::min(totalMoves, 63)] + moveHistory / 8192);
        ...
        if (ss->ply < thread_data->rootDepth * 2) {
            if (!isRootNode && depth >= 7 && move == tt_data.move && !excludedMove_val && (tt_data.bound & LOWER) &&
                ...
                int singular_score      = search<false>(singular_beta - 1, singular_beta, singularDepth, cutNode, thread_data, ss);
    ...
    if (!excludedMove.data()) {
        if (!inCheck && (!best_move.data() || !is_tactical(best_move)) && !(bound == LOWER && bestScore <= ss->staticEval) &&
            !(bound == UPPER && bestScore >= ss->staticEval)) {
            update_corrHistScore(pos, search_data, depth, bestScore - ss->staticEval);
        }

        TT_data new_data(rawEval, bound, best_move.data(), /*this needs to change*/ 0, pos->position_key, depth);
        tt.save_entry(tt_entry->save(new_data));
    }
...
template <bool pvNode> int Quiescence(int alpha, int beta, ThreadData* thread_data, SearchStack* ss) {
    ...
    if (!pvNode && tt_data.value != SCORE_NONE &&
        ((tt_data.bound == UPPER && tt_data.value <= alpha) || (tt_data.bound == LOWER && tt_data.value >= beta) ||
         tt_data.bound == EXACT)) {
    ...
    if (inCheck) {
        ...
        if (tt_data.value != SCORE_NONE && ((tt_data.bound == UPPER && tt_data.value < best_score) ||
