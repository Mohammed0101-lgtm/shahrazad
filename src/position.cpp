...
Bitboard Position::_black_occupancy() const {
    Bitboard occ = black_bishops.board() | black_king.board() | black_knights.board() | black_pawns.board() | black_queens.board() |
                   black_rooks.board();
...
Bitboard Position::_white_occupancy() const {
    Bitboard occ = white_bishops.board() | white_king.board() | white_knights.board() | white_pawns.board() | white_queens.board() |
                   white_rooks.board();
...
uint8_t  Position::king_square(int color) const { return color == WHITE ? white_king.square() : black_king.square(); }
...
uint8_t Position::isPinned(const int sq) const {
    ...
    for (int op_sq = 0; op_sq < 64 && opponent_occupancy.is_bitset(op_sq); op_sq++) {
        switch (pieces[op_sq]) {
        case BISHOP: {
            std::vector<uint8_t> moves = bishop_moves(sq, current_side == WHITE ? BLACK : WHITE, *this);
        ...
        case QUEEN: {
            std::vector<uint8_t> moves = queen_moves(sq, current_side == WHITE ? BLACK : WHITE, *this);
        ...
        case ROOK: {
            std::vector<uint8_t> moves = rook_moves(sq, current_side == WHITE ? BLACK : WHITE, *this);
