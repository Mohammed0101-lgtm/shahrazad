...
int get_history_score(
    const Position& pos, SearchData* search_data, const Move& move,
    SearchStack* ss) {}

void update_corrHistScore(
    const Position* pos, SearchData* search_data, int depth, int score) {}

void update_histories(
    const Position* pos, SearchData* search_data, SearchStack* ss, int depth,
    const Move& best_move, MoveList* quietMoves, MoveList* noisyMoves) {}

int  get_single_score(SearchStack* ss, Move& move, int offset) {}

void update_score(
    SearchStack* ss, Move& move, int bonus, const int offset,
    const Position& pos) {
    if ((ss - offset)->move.asShort()) {
        const int scaledBonus = bonus - get_single_score(ss, move, offset) *
                                            std::abs(bonus) / CH_MAX;
        (*((ss - offset)->contHistEntry))[pos.pieceOn(move.getFrom())] +=
            scaledBonus;
    }
}
...
bool SEE(const Position pos, Move& move, int thresh_hold) {
    ...
    Bitboard our_occ =
        side == WHITE ? pos._white_occupancy() : pos._black_occupancy();
    ...
    while (true) {
        ...
        for (piece_type = PAWN; piece_type < KING;
            ...
            if (OurAttackers.board() &
                get_piece_by_type(&pos, piece_type).board())
                break;
        }
...
template <bool pvNode>
int search(
    int alpha, int beta, int depth, const bool cutNode, ThreadData* thread_data,
    SearchStack* ss) {
    ...
    if (!pvNode && tt_data.value != SCORE_NONE && tt_data.depth >= depth &&
        ((tt_data.bound == UPPER && tt_data.value <= alpha) ||
         (tt_data.bound == LOWER && tt_data.value >= alpha) ||
         tt_data.bound == EXACT)) {
        return tt_data.eval;
    }
    ...
    if (inCheck) {
        ...
        rawEval = (tt_data.value != SCORE_NONE)
                      ? tt_data.eval
                      : network_eval(*pos, nnue, caches);
        ...
        if (tt_data.eval != SCORE_NONE &&
            ((tt_data.bound == UPPER && tt_data.eval < eval) ||
             (tt_data.bound == LOWER && tt_data.eval > eval) ||
             tt_data.bound == EXACT)) {
            eval = tt_data.eval;
        }
    } else {
    ...
    if (!pvNode && !excludedMove_val && !inCheck) {
        if (depth < 10 && abs(eval) < MATE_FOUND &&
            eval - 91 * (depth - improve) >= beta)
            return eval;

        Bitboard pawns =
            pos->current_side == WHITE ? pos->white_pawns : pos->black_pawns;
        bool no_pawns = (pawns.board() == 0ULL);

        if (eval >= ss->staticEval && eval >= beta &&
            (ss - 1)->move != NOMOVE && ss->ply >= thread_data->nmpPlies &&
            no_pawns) {
            ss->move    = NOMOVE;
            ...
            int nmpScore = -search<false>(
                -beta, -beta + 1, depth - R, !cutNode, thread_data, ss + 1);
            ...
            if (nmpScore >= beta) {
                ...
                int verificationScore = search<false>(
                    beta - 1, beta, depth - R, false, thread_data, ss);
        ...
        if (depth <= 5 && eval + 256 * depth < alpha) {
            const int razorScore =
                Quiescence<false>(alpha, beta, thread_data, ss);
    ...
    while ((move = move_picker.next(skipQuiets)) != NOMOVE) {
        ...
        if (!isRootNode && no_pawns && bestScore > -MATE_FOUND) {
            const int lmrDepth = std::max(
                0, depth -
                       reductions[isQuiet][std::min(depth, 63)]
                                 [std::min(totalMoves, 63)] +
                       moveHistory / 8192);

            if (!skipQuiets) {
                if (!pvNode && !inCheck &&
                    totalMoves > lmp_margin[std::min(depth, 63)][improve])
                    skipQuiets = true;
        ...
        if (ss->ply < thread_data->rootDepth * 2) {
            if (!isRootNode && depth >= 7 && move == tt_data.move &&
                !excludedMove_val && (tt_data.bound & LOWER) &&
                abs(tt_data.value) < MATE_SCORE && tt_data.depth >= depth - 3) {
                const int singular_beta = tt_data.value - depth;
                const int singularDepth = (depth - 1) / 2;

                ss->excludedMove        = tt_data.move;
                int singular_score      = search<false>(
                    singular_beta - 1, singular_beta, singularDepth, cutNode,
                    thread_data, ss);
                ...
                if (singular_score < singular_beta) {
                    ...
                    if (!pvNode && singular_score < singular_beta - 17 &&
                        ...
                        extension = 2 + (!is_tactical(tt_data.move) &&
                                         singular_score < singular_beta - 100);
        ...
        pos->do_move(move);
        ss->contHistEntry =
            &search_data->contHist[pos->pieceOn(move.getFrom())];
        list.append(move);
        info->nodes++;
        const uint64_t nodes_before_search = info->nodes;

        if (totalMoves > 1 + pvNode && depth >= 3 && (isQuiet)) {
            int depth_reduction = reductions[isQuiet][std::min(depth, 63)]
                                            [std::min(totalMoves, 63)];
            ...
            score             = -search<false>(
                -alpha - 1, -alpha, reduced_depth, true, thread_data, ss + 1);
            ...
            if (score > alpha && new_depth > reduced_depth) {
                ...
                if (new_depth > reduced_depth)
                    score -= search<false>(
                        -alpha - 1, -alpha, new_depth, !cutNode, thread_data,
                        ss + 1);
            ...
            score = -search<false>(
                -alpha - 1, -alpha, new_depth, !cutNode, thread_data, ss + 1);
        }

        if (pvNode && (totalMoves == 1 || score > alpha))
            score = -search<true>(
                -beta, -alpha, new_depth, false, thread_data, ss + 1);

        pos->undo_move(move);

        if (thread_data->id == 0 && isRootNode)
            thread_data->nodeSpentTable[move.getFrom()] +=
                info->nodes - nodes_before_search;
        ...
        if (score > bestScore) {
            ...
            if (score > alpha) {
                ...
                if (pvNode) {
                    ...
                    for (int next = ss->ply + 1;
                         next < pv_table->pvLength[ss->ply + 1]; next++)
                        pv_table->pvArray[ss->ply][next] =
                            pv_table->pvArray[ss->ply + 1][next];

                    pv_table->pvLength[ss->ply] =
                        pv_table->pvLength[ss->ply + 1];
                }
                ...
                if (score >= beta) {
                    if (isQuiet) {
                        ...
                        if (ss->ply >= 1)
                            search_data
                                ->counterMoves[(ss - 1)->move.getFrom()] =
                                move.asShort();
                    }

                    update_histories(
                        pos, search_data, ss, depth + (eval <= alpha),
                        best_move, &quietMoves, &noisyMoves);
    ...
    if (totalMoves == 0)
        return excludedMove_val ? -MAXSCORE
    ...
    if (!excludedMove.asShort()) {
        if (!inCheck && (!best_move.asShort() || !is_tactical(best_move)) &&
            ...
            update_corrHistScore(
                pos, search_data, depth, bestScore - ss->staticEval);
        }
...
int Quiescence(int alpha, int beta, ThreadData* thread_data, SearchStack* ss) {
    ...
    if (!pvNode && tt_data.value != SCORE_NONE &&
        ((tt_data.bound == UPPER && tt_data.value <= alpha) ||
         (tt_data.bound == LOWER && tt_data.value >= beta) ||
         tt_data.bound == EXACT)) {
    ...
    if (inCheck) {
        ...
        rawEval        = tt_data.eval != SCORE_NONE ? tt_data.eval
                                                    : network_eval(*pos, nnue, caches);
        ss->staticEval = best_score =
            adjustEvalWithCorrHist(pos, search_data, rawEval);
    ...
    while ((move = move_picker.next(!inCheck || best_score > -MATE_FOUND)) !=
           NOMOVE) {
        if (!isLegal(*pos, move))
            continue;
        ...
        info->nodes++;
        const int score =
            -Quiescence<pvNode>(-beta, -alpha, thread_data, ss + 1);
